/*
 * Public API Surface of ngx-configuration
 */

// Export models / enums
export {CgtDictionary} from './lib/models/cgt-dictionary';
export {CgtDictionaryValue} from './lib/models/cgt-dictionary-value';

// Export Pipes
export {CgtDictionaryPipe} from './lib/pipes/cgt-dictionary.pipe';

// Export Components
export {CgtDictionarySelectComponent} from './lib/components/select/cgt-dictionary-select.component';

// Export services
export {CgtDictionaryService, CgtDictionaryServiceConfig} from './lib/services/cgt-dictionary.service';
export {CgtDictionaryValueService} from './lib/services/cgt-dictionary-value.service';

// Export modules
export {CgtDictionaryModule} from './lib/cgt-dictionary.module';
